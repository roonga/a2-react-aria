...
export default function AdminPage() {
	...
	return (
		<div>
			...
			{loading ? (
				<p className="text-(--color-textMuted) text-sm">Loading…</p>
			) : surveys.length === 0 ? (
				<p className="text-(--color-textMuted) text-sm">No surveys yet.</p>
			) : (
					...
					{surveys.map((s) => (
						...
						>
							<div className="min-w-0 flex-1">
								...
								{s.description && <p className="mt-0.5 truncate text-(--color-textMuted) text-sm">{s.description}</p>}
								<p className="mt-0.5 text-(--color-textMuted) text-xs">{s.step_count ?? 0} steps</p>
							</div>
							<div className="flex shrink-0 items-center gap-2">
								...
								<button
									...
									className="rounded-md border border-(--color-danger) px-3 py-1.5 text-(--color-danger) text-sm hover:bg-(--color-backgroundMuted)"
								>
									Delete
								</button>
							</div>
