export const getFeedbackSurveyStyles = () => ({
	...
	description: "text-sm text-(--color-textMuted) -mt-2",
	...
	starButton: (active: boolean) =>
		[
			...
			active ? "text-(--color-star) scale-110" : "text-(--color-textMuted) hover:text-(--color-star) hover:scale-110",
		].join(" "),
	textarea: [
		"w-full rounded-lg border border-(--color-border) bg-(--color-background)",
		"text-sm text-(--color-text) placeholder:text-(--color-textMuted)",
	...
	submit: (enabled: boolean) =>
		enabled
			? [
					...
					"text-(--color-primaryForeground) bg-(--color-primary) hover:bg-(--color-primaryHover)",
					"active:bg-(--color-primaryActive) transition-colors cursor-pointer",
					"focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-(--color-primary)",
				].join(" ")
			: [
					"inline-flex items-center justify-center rounded-lg px-4 py-2 text-sm font-medium",
					"text-(--color-textMuted) bg-(--color-backgroundMuted)",
					"cursor-not-allowed opacity-60",
				].join(" "),
	thanksWrap: "flex flex-col items-center gap-2 py-4 text-center",
	thanksText: "text-base font-semibold text-(--color-text)",
	thanksSub: "text-sm text-(--color-textMuted)",
})
