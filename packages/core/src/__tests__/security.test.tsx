...
describe("A03 URL injection — javascript: scheme is blocked regardless of casing/whitespace", () => {
	...
	it("blocks javascript: href with a tab embedded inside the scheme name", () => {
		// Browsers strip ASCII tab/newline/CR from anywhere in a URL before parsing the
		// scheme, so "jav\tascript:" is parsed as "javascript:". A filter that only
		// trims the ends misses this.
		const { container } = render(
			<A2Renderer
				node={{ type: "Anchor", props: { href: "jav\tascript:alert(1)" }, children: "x" }}
				registry={registry}
			/>,
		)
		expect(container.querySelector("a")?.getAttribute("href")).toBe("about:blank")
	...
	it("blocks javascript: href split by multiple embedded control characters", () => {
		const { container } = render(
			<A2Renderer
				node={{ type: "Anchor", props: { href: "j\ta\nv\ra\tscript:alert(1)" }, children: "x" }}
				registry={registry}
			/>,
		)
		expect(container.querySelector("a")?.getAttribute("href")).toBe("about:blank")
